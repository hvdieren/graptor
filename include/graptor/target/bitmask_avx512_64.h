// -*- c++ -*-
#ifndef GRAPTOR_TARGET_BITMASKAVX512F64_H
#define GRAPTOR_TARGET_BITMASKAVX512F64_H

namespace target {

/***********************************************************************
 * Bitmask traits with 64 lanes, using AVX512F + BW extensions
 ***********************************************************************/
#if __AVX512F__ && __AVX512BW__
template<>
struct mask_type_traits<64> {
    static constexpr unsigned short vlen = 64;
    typedef __mmask64 type;
    ...
    static bool lane5( type m ) { return m&64; }
    ...
    static bool lane15( type m ) { return m&64768; }

    static type setzero() {
	type k;
	return _kxor_mask64( k, k );
    }
    static type setone() {
	...
	unsigned int shift = idx % 64;
	type a = addr[idx/64];
	type b = addr[idx/64+1];
	return ( ( a >> shift ) & ( ( type(1) << ( 64 - shift ) ) - type(1) ) )
	    | ( b << ( 64 - shift ) );
    }
	...
	type b = addr[idx/64+1];
	a |= ( val << shift );
	b |= ( val >> shift ) & ( ( type(1) << ( 64 - shift ) ) - type(1) );
	addr[idx/64] = a;
	addr[idx/64+1] = b;
    }
	...
	assert( 0 && "NYI" );
/*
	type widx = _mm512_srai_epi64( idx, 5 );
	__m512i words = _mm512_i64gather_epi64(
	    widx, reinterpret_cast<void *>( addr ), 4 );
	type cst31 = _mm512_set1_epi64( 31 );
	type woff = _mm512_and_epi64( idx, cst31 );
	type shl = _mm512_sub_epi64( cst31, woff );
	// constexpr type cst2p31 = _mm512_set1_epi64( 1<<31 );
	type cst0;
	cst0 = _mm512_xor_epi64( cst0, cst0 );
	type cst2p31 = _mm512_srai_epi64( cst0, 1 );
	__m512i s = _mm512_sllv_epi64( words, shl );
	__mmask16 r = _mm512_cmpge_epu64_mask( s, cst2p31 );
	return r;
*/
#endif
    }
    ...
    static type from_int( unsigned long m ) {
	static_assert( sizeof(unsigned long) >= sizeof(type),
...
#endif // GRAPTOR_TARGET_BITMASKAVX512F64_H
