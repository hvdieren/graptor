...
#ifndef GRAPTOR_GRAPH_NUMA_H
#define GRAPTOR_GRAPH_NUMA_H
...
#include <sys/mman.h>
#if NUMA
#include "graptor/numa_page_check.h"
#include <numaif.h>
#include <numa.h>
#endif
...
inline Edge::Edge( const Edge_Hilbert & e ) : src( e.src ), dst( e.dst )
#ifdef WEIGHTED
        , weight( e.weight )
#endif
    { }
...
//    asymmetricVertex * newV = new asymmetricVertex [n];
    ...
    intT numVertices() const { return n; }
    intT numEdges() const { return m; }
};
...
public:
    ...
    const EdgeList<Edge> & get_edge_list_partition( intT p ) const
    {
        return localEdgeList[p];
    }

    const graph<vertex> & get_partition() const
    {
        return CSCGraph;
    }
    const graph<vertex> & getCSC() const { return get_partition(); }
