...
bool test() {
    ...
    mask_type c = tr::BINOP( a, b, target::mt_mask() );

    for( unsigned short l=0; l < VL; ++l )
	cvals[l] = str::BINOP( avals[l], bvals[l], target::mt_mask() );
    ...
    unsigned short cdiff = mask_compare<tr>( c, cvals );
    if( cdiff != 0 ) {
	...
	show<tr>( b, bvals );
	std::cerr << "vector C:\n";
	show<typename tr::mask_traits>( c, cvals );
    }
