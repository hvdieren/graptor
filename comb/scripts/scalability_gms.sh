...
function get_avg() {
	...
	if [ -e $file ] ; then
		...
		local val=`( grep "Average Time:" $file 2> /dev/null || echo Average Time: ABORT ) | tr -s ' ' | cut -d' ' -f3 | sed -e 's/s$//' | head -n 4 | tail -1`
...
function mce() {
	...
	for graph in $graphs ; do
		...
		one gms $thr maximal_clique_enum_bron_kerbosch ${graph}
	    done
	done
